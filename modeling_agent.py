...
class ModelingAgent(Agent):
    ...
    def generate(
        ...
        resp = self.llm.generate(prompt, model=model, max_tokens=6000, worker="Modeling")
        response_text = resp.get("text", "")

        result = self._extract_json(response_text)

        if isinstance(result, list):
            result = {"problem": problem_type or "classification", "target": target or "target", "models": result, "code": ""}
    ...
    def _extract_code_from_response(self, text: str) -> dict:
        ...
        if code_match:
            ...
            return {"problem": "classification", "target": "target", "models": ["RandomForest"], "code": code}
        return {}
